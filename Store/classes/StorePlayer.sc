StoreRoutine {
	...
	*new { arg store, start, ignoreLoop;
		^super.newCopyArgs(store, ignoreLoop ? false).init(start);
	}
	
	init { arg start;
		^this.getRoutine(start ? 0);
	}
...
// 	*new { arg store;
// 		// ^instance !? { instance.setStore(store).init() } ?? {
// 		// 	instance = super.newCopyArgs(store).init();
// 		// 	^instance;
// 		// }
...
// 	getRoutine { arg start, loopPoints;
// 		^Routine({ arg inval;
// 			var firstEvents = this.getNextEventGroup(
// 				store.getItems,
// 				start,
